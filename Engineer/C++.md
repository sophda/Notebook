...
运行时是程序运行起来之后，为其提供必要的支持环境、库、服务或机制的总称。它不包含程序本身的业务逻辑，但是没有的话程序无法跑起来，或者无法完成正常的核心功能（如内存管理、异常处理、依赖调用等）。

c++运行时（c++ runtime，简称CRT），是支持c++程序运行的基础库+核心机制，核心职责是**封装底层细节、让c++程序可以跨平台且安全的运行**

1.核心作用包括：

- 管理程序启动与退出，完成各种全局变量静态变量的初始化之后，才调用main，结束时清理资源（如销毁 全局变量，关闭标准输入输出流）
- 调度内存管理，使用的`new delete malloc free`等，底层都是调用的c++运行时的内存管理模块，负责向操作系统申请内存、划分内存块、回收无用的内存、处理内存对齐、小块内存池优化等
- 异常处理，`try-catch-throw`完全依赖运行时来实现
- 处理标准库，vector的数组内存管理依赖运行时
- 处理跨平台，std::fopen的用法一致，但是在Linux和Windows上调用的操作系统接口不同
  - 在Linux上调用的是open系统调用
  - 在Windows上调用的是CreateFile

2.运行时的形态

- 静态库、动态库，编译时会链接到程序中。如Windows平台的libcmt.lib（静态库）。或者Linux平台的动态库，如libstdc++.so（动态库）。
- 精简版c++运行时，如嵌入式场景的newlib
...
![image-20250607150407629](assets/C++_img/image-20250607150407629.png)

---

![image-20250607150918247](assets/C++_img/image-20250607150918247.png)

---

![image-20250607150950016](assets/C++_img/image-20250607150950016.png)
...
![image-20231204001558057](assets/C++_img/image-20231204001558057.png)



### 双目运算符

![image-20231204001740918](assets/C++_img/image-20231204001740918.png)
...
![image-20231124130027398](assets/C++_img/image-20231124130027398.png)
...
>
> ![image-20230319225242197](assets/C++_img/image-20230319225242197.png)
...
![image-20250607151428854](assets/C++_img/image-20250607151428854.png)

![image-20250607152328220](assets/C++_img/image-20250607152328220.png)
...
![image-20250607151910600](assets/C++_img/image-20250607151910600.png)
...
![image-20250607152014083](assets/C++_img/image-20250607152014083.png)

### const_cast

谨慎使用

![image-20250607152129368](assets/C++_img/image-20250607152129368.png)
...
![image-20250607152201740](assets/C++_img/image-20250607152201740.png)
...
![](assets/C++_img/2023-02-14-02-23-06-image.png)
...
![](assets/C++_img/2023-02-14-02-27-00-image.png)
...
![](assets/C++_img/2023-02-14-02-30-06-image.png)
...
- 如果是**类的对象**
  ...
  ![image-20231123002044059](assets/C++_img/image-20231123002044059.png)
...
- 如果**类的对象存在于{}，也就是栈中**
  ...
  ![image-20231123002259270](assets/C++_img/image-20231123002259270.png)
...
- 如果**类的对象申请在堆上，也就是new**
  ...
  ![image-20231123002526301](assets/C++_img/image-20231123002526301.png)
  ...
  ![image-20231123003341993](assets/C++_img/image-20231123003341993.png)

  **因为a这个指针存在于栈上，所以超出了作用域，a这个指针是消失了，但是内存并没有被释放。所以在{}空间内，也就是栈中（常见的比如：函数等）不要去new一个内存**

  所以这个a指针，需要手动去delete：

  ![image-20231123003434743](assets/C++_img/image-20231123003434743.png)

  ![image-20231123003445901](assets/C++_img/image-20231123003445901.png)
...
![image-20250504002708502](assets/C++_img/image-20250504002708502.png)
...
2. 类型的成员变量
   ...
   ![image-20230801084432660](assets/C++_img/image-20230801084432660.png)
...
3. 类的成员函数
   ...
   ![image-20230801084735972](assets/C++_img/image-20230801084735972.png)
...
4. 类的私有成员
   ...
   ![image-20230801084956653](assets/C++_img/image-20230801084956653.png)
...
5. 虚函数
   ...
   ![image-20230801085854130](assets/C++_img/image-20230801085854130.png)
...
![image-20250715005526027](assets/C++_img/image-20250715005526027.png)
...
- 调用`string s = "hello"; g(s);`时，s是个左值，推导为s类型的引用即T=string&，此时g函数的实例化版本为：
  ...
  **因为出现了引用的引用，因此需要引用折叠规则：**
  ...
  ![image-20250715010544789](assets/C++_img/image-20250715010544789.png)
...
![image-20250716172140698](assets/C++_img/image-20250716172140698.png)
...
![image-20250908011152183](assets/C++_img/image-20250908011152183.png)
...
![image-20231118024706561](assets/C++_img/image-20231118024706561.png)
...
- ![image-20250715004909237](assets/C++_img/image-20250715004909237.png)
