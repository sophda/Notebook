...
1. 工作空间
   ...
   ![](src/SLAM/2023-02-14-03-40-18-image.png)

2. 创建工作空间
  
   ![](src/SLAM/2023-02-14-03-40-42-image.png)

3. 使用`catkin_create_pkg`创建包，依赖于`std_msgs`,`rospy`，`roscpp`
  
   ![](src/SLAM/2023-02-14-03-42-00-image.png)
   
   执行上述命令后，会生成`beginer_turorials`文件夹，包含`cmakelists.txt`和`package.xml`

4. 编译包
  
   ![](src/SLAM/2023-02-14-03-43-17-image.png)

5. 添加安装文件
  
   ![](src/SLAM/2023-02-14-03-43-54-image.png)

## 包依赖

- `fist-order dependencies`

![](src/SLAM/2023-02-14-03-44-46-image.png)

`rospack`列出了`catkin_create_pkg`使用的依赖，这些以来在`package.xml`文件中展示

- `indirect dependencies`间接依赖

![](src/SLAM/2023-02-14-03-46-46-image.png)
...
1. 定制`package.xml`
   ...
   ![](src/SLAM/2023-02-14-03-47-58-image.png)
   
   ![](src/SLAM/2023-02-14-03-48-10-image.png)
   
   ![](src/SLAM/2023-02-14-03-48-24-image.png)
   
   ![](src/SLAM/2023-02-14-03-48-36-image.png)
...
![](src/SLAM/2023-02-14-04-13-06-image.png)

### 问题

1. 提示

![](src/SLAM/2023-02-14-04-14-36-image.png)
...
2. 包问题
   ...
   ![](src/SLAM/2023-02-14-04-15-21-image.png)
...
3. lboost问题
   ...
   ![](src/SLAM/2023-02-14-04-17-39-image.png)

**结果：**

![image-20230606230423975](src/SLAM/image-20230606230423975.png)
...
![image-20231002142018343](src/SLAM/image-20231002142018343.png)
...
![image-20240411010552905](src/SLAM/image-20240411010552905.png)
...
![image-20241105011444804](src/SLAM/image-20241105011444804.png)

这里作者假定加速度以及陀螺仪测量的**addiive noise**为正态分布nba和nbw服从，也就是下一次的a与w与上次的差值服从一个正态分布。

于是：a和w的偏差的导数有：

![image-20241105011707056](src/SLAM/image-20241105011707056.png)

2. 预积分（local frame）

预积分对两个连续时间的帧bk与bk+1，由于imu的测量帧率大于照片的速率，于是在两帧之间有很多imu测量值。在local frame中有bk：

![image-20241105012026088](src/SLAM/image-20241105012026088.png)



## 估计器初始化--在滑动窗口中的纯视觉sfm

在滑动窗口中放很多帧：

- 首先，检查最新帧和以前所有帧的对应关系，如果有稳定的特征跟踪（大于30个特征点），并且最新帧与滑动窗口其他帧之间有足够的视差，使用5点算法恢复两帧之间的相对旋转和缩放平移。
- 然后，任意设置尺度，对两帧中观察到的特征进行三角测量，基于三角特征执行透视点pnp方法估计窗口中其他帧的状态。
- 最后应用全局全束调整最小化所有特征观测的重投影误差。
- **由于没有世界坐标系的位置，所以将第一个相机坐标系$(·)^{c0}$设置为sfm的参考坐标系** 
...
对于窗口中两个连续的帧，bk和bk+1，得到了基于c0坐标系的旋转$q_{bk}^{c0}$,以及$q_{bk+1}^{c0}$，以及相对约束
...
主要记住编译指令，在交叉编译ceres的时候开启了openmp，那么在把ceres链接到其他动态库上的时候，也要链接openmp这个动态库。
...
add_subdirectory(
    /home/sophda/project/VinsAR/VINS-Mobile-master
    /home/sophda/project/VinsAR/VINS-Mobile-master/build
)
# vins include
include_directories(
    /home/sophda/project/VinsAR/VINS-Mobile-master/VINS_ios
    /home/sophda/project/VinsAR
)
...
target_link_options(vinsar PRIVATE
-fopenmp)
```
